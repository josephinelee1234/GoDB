...
func (a *Aggregator) Descriptor() *TupleDesc {
	...
	tempField := make([]FieldType, len(a.groupByFields))
	for i := 0; i < len(a.groupByFields); i++ {
		tempField[i] = a.groupByFields[i].GetExprType()
	}
	resultDesc := &TupleDesc{Fields: tempField}
	for i := 0; i < len(a.newAggState); i++ {
		resultDesc = a.newAggState[i].GetTupleDesc().merge(resultDesc)
	}
	return resultDesc
}
...
func extractGroupByKeyTuple(a *Aggregator, t *Tuple) (*Tuple, error) {
	// TODO: some code goes here
	eval := &Tuple{}
	numFields := len(a.groupByFields)
	for i := 0; i < numFields; i++ {
		groups := a.groupByFields[i]
		eval.Desc.Fields = append(eval.Desc.Fields, groups.GetExprType())
		value, err := groups.EvalExpr(t)
		if err != nil {
			return eval, err
		}
		eval.Fields = append(eval.Fields, value)
...
func getFinalizedTuplesIterator(a *Aggregator, groupByList []*Tuple, aggState map[any]*[]AggState) func() (*Tuple, error) {
	...
	return func() (*Tuple, error) {
		if currentTupleIndex >= len(groupByList) {
			return nil, nil // End of the list
		}

		currentTuple := groupByList[currentTupleIndex]
		aggList, found := aggState[currentTuple.tupleKey()]

		if found == false {
			return nil, nil
		}

		if found && aggList != nil {
			for i := 0; i < len(*aggList); i++ {
				item := (*aggList)[i]
				finalizedTuple := item.Finalize()
				currentTuple = joinTuples(currentTuple, finalizedTuple)
			}
		}
		currentTupleIndex++
